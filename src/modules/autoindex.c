...
fh_autoindex_handle (struct fh_autoindex *autoindex)
{
	...
	for (int i = 0; i < namelist_count; i++)
	{
		...
		else
		{
			...
			int full_path_size = snprintf (full_path, sizeof full_path, "%s/%s",
										   autoindex->filename, entry->d_name);
			...
			if (stat64 (full_path, &st) < 0)
			{
				fh_pr_debug ("Failed to stat '%s': %s", full_path,
							 strerror (errno));
				goto next_iter;
			}
			...
			if (!tm)
			{
				fh_pr_debug ("Failed to get mtime for '%s': %s", full_path,
							 strerror (errno));
				goto next_iter;
			}

			char time_str[24] = { 0 };
			size_t time_len = strftime (time_str, (sizeof time_str) - 1,
										"%Y-%m-%d %H:%M:%S", tm);

			if (time_len == 0)
			{
				fh_pr_debug ("Failed to format mtime for '%s': %s", full_path,
							 strerror (errno));
				goto next_iter;
			}

			char size_buf[64] = { 0 };
			size_t size_buf_len = 0;

			if (!is_dir)
			{
				if (!format_size ((size_t) st.st_size, size_buf, NULL, NULL,
								  &size_buf_len))
				{
					response->status = FH_STATUS_INTERNAL_SERVER_ERROR;
					goto ret_clean;
			...
			size_t chunk_len
				= format_len - 19 - 4 + (2 * name_len)
				  + (is_dir ? 6 + 3 + 2 + 1 : (4 + 4 + size_buf_len))
				  + time_len;
			...
			if ((rc = snprintf ((char *) link->buf->attrs.mem.data, len + 1,
								...
								is_dir ? "/" : "", entry->d_name,
								is_dir ? "/" : "", is_dir ? "-" : size_buf,
								time_str))
				< 0)
			{
				fh_pr_debug ("Failed to format entry for '%s': %s", full_path,
							 strerror (errno));
				goto next_iter;
			}
