...
#define fhttpd_wlog_debug(pid, format, ...)                                    \
    fhttpd_log (FHTTPD_LOG_LEVEL_DEBUG, "[Worker %d] " format, pid,            \
                ##__VA_ARGS__)
#define fhttpd_wlog_info(pid, format, ...)                                     \
    fhttpd_log (FHTTPD_LOG_LEVEL_INFO, "[Worker %d] " format, pid,             \
                ##__VA_ARGS__)
#define fhttpd_wlog_warning(pid, format, ...)                                  \
    fhttpd_log (FHTTPD_LOG_LEVEL_WARNING, "[Worker %d] " format, pid,          \
                ##__VA_ARGS__)
#define fhttpd_wlog_error(pid, format, ...)                                    \
    fhttpd_log (FHTTPD_LOG_LEVEL_ERROR, "[Worker %d] " format, pid,            \
                ##__VA_ARGS__)
#define fhttpd_wlog_fatal(pid, format, ...)                                    \
    fhttpd_log (FHTTPD_LOG_LEVEL_FATAL, "[Worker %d] " format, pid,            \
                ##__VA_ARGS__)
