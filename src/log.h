...
#define fhttpd_wlog_debug(pid, format, ...)                                    \
    fhttpd_log (FHTTPD_LOG_LEVEL_DEBUG, "[Worker %d] " _FHTTPD_LOG_MODULE_NAME format, pid,            \
                ##__VA_ARGS__)
#else
#define fhttpd_wlog_debug(pid, format, ...)
#endif
...
#define fhttpd_wlog_info(pid, format, ...)                                     \
    fhttpd_log (FHTTPD_LOG_LEVEL_INFO, "[Worker %d] " _FHTTPD_LOG_MODULE_NAME format, pid,             \
                ##__VA_ARGS__)
#define fhttpd_wlog_warning(pid, format, ...)                                  \
    fhttpd_log (FHTTPD_LOG_LEVEL_WARNING, "[Worker %d] " _FHTTPD_LOG_MODULE_NAME format, pid,          \
                ##__VA_ARGS__)
#define fhttpd_wlog_error(pid, format, ...)                                    \
    fhttpd_log (FHTTPD_LOG_LEVEL_ERROR, "[Worker %d] " _FHTTPD_LOG_MODULE_NAME format, pid,            \
                ##__VA_ARGS__)
#define fhttpd_wlog_fatal(pid, format, ...)                                    \
    fhttpd_log (FHTTPD_LOG_LEVEL_FATAL, "[Worker %d] " _FHTTPD_LOG_MODULE_NAME format, pid,            \
                ##__VA_ARGS__)

#define fhttpd_wlog_perror(pid, format, ...)                                   \
    fhttpd_log (FHTTPD_LOG_LEVEL_ERROR, "[Worker %d] " format ": %s", pid,     \
