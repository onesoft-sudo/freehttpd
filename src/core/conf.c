								   ...
								   const struct conf_node *value)
{
	...
	int level = strcasecmp (value->details.literal.value.str.value, "debug") == 0	? LOG_DEBUG
				: strcasecmp (value->details.literal.value.str.value, "info") == 0	? LOG_INFO
				: strcasecmp (value->details.literal.value.str.value, "warn") == 0	? LOG_WARN
				: strcasecmp (value->details.literal.value.str.value, "error") == 0 ? LOG_ERR
																					: -1;
...
fhttpd_conf_print_config (const struct fhttpd_config *config, int indent)
{
	...
	fh_pr_debug ("%*ssrv_root: %s", (int) indent, "", config->conf_root ? config->conf_root : "(null)");
	fh_pr_debug ("%*sdocroot: %s", (int) indent, "", config->docroot ? config->docroot : "(null)");
	fh_pr_debug ("%*sworker_count: %zu", (int) indent, "", config->worker_count);

	fh_pr_debug ("%*slogging.min_level: %d", (int) indent, "", config->logging_min_level);
	fh_pr_debug ("%*slogging.file: %s", (int) indent, "", config->logging_file ? config->logging_file : "(null)");
	fh_pr_debug ("%*slogging.error_file: %s", (int) indent, "",
					  config->logging_error_file ? config->logging_error_file : "(null)");

	fh_pr_debug ("%*ssecurity.recv_timeout: %u", (int) indent, "", config->sec_recv_timeout);
	fh_pr_debug ("%*ssecurity.send_timeout: %u", (int) indent, "", config->sec_send_timeout);
	fh_pr_debug ("%*ssecurity.header_timeout: %u", (int) indent, "", config->sec_header_timeout);
	fh_pr_debug ("%*ssecurity.body_timeout: %u", (int) indent, "", config->sec_body_timeout);
	fh_pr_debug ("%*ssecurity.max_response_body_size: %zu", (int) indent, "", config->sec_max_response_body_size);
	fh_pr_debug ("%*ssecurity.max_connections: %zu", (int) indent, "", config->sec_max_connections);
	...
	for (size_t i = 0; i < config->host_count; i++)
	{
		...
		{
			fh_pr_debug ("%*s  Bound address: %s:%u", (int) indent, "", host->bound_addrs[j].hostname,
							  host->bound_addrs[j].port);
		}

		fh_pr_debug ("%*s  Host Configuration:", (int) indent, "");
