...
http1_parser_ctx_free (struct http1_parser_ctx *ctx)
{
	if (!ctx->result.used)
	{
		free (ctx->result.host);
		free (ctx->result.full_host);
...
http1_parse_header_value (struct http1_parser_ctx *ctx)
{
	...
	else if (strcasecmp (hname_str, "Host") == 0)
	{
		...
		size_t port_len = colon ? strlen (colon + 1) : 2;
		size_t full_host_len = ctx->result.host_len + port_len + 2;
		ctx->result.full_host = malloc (full_host_len);

		if (!ctx->result.full_host)
		{
			free (hname_str);
			free (hvalue_str);
			fhttpd_wclog_debug ("Memory allocation failed");
		...
		memcpy (ctx->result.full_host, ctx->result.host, ctx->result.host_len);
		ctx->result.full_host[ctx->result.host_len] = ':';

		/** FIXME: TLS */
		memcpy (ctx->result.full_host + ctx->result.host_len + 1, colon ? colon + 1 : "80", port_len);
		ctx->result.full_host[full_host_len - 1] = 0;
		ctx->result.full_host_len = full_host_len;
	}
...
{
	if (!ctx->result.host || !ctx->result.full_host)
	{
		fhttpd_wclog_debug ("No Host header was found in the request");
		ctx->state = HTTP1_STATE_ERROR;
		return HTTP1_PARSER_RETURN (false);
	}
					   ...
					   const struct fhttpd_response *response)
{
	...
	while (ctx->buffer_len < HTTP1_RESPONSE_BUFFER_SIZE)
	{
		...
		if (response->use_builtin_error_response)
		{
			...
			size_t host_len = conn->hostname ? conn->hostname_len : 7;
			...
			const size_t content_length = (strlen (status_text) * 2) + (2 * 3) + host_len + port_width
										  + strlen (description) + resource_error_html_len - 14;
			...
			char format[82 + resource_error_html_len + 1];

			memcpy (format,
					"Server: freehttpd\r\nContent-Type: text/html; charset=UTF-8\r\nContent-Length: %zu\r\n\r\n", 82);
			memcpy (format + 82, (const char *) resource_error_html, resource_error_html_len);

			format[82 + resource_error_html_len] = 0;

			int sp_bytes = snprintf (ctx->buffer + ctx->buffer_len, HTTP1_RESPONSE_BUFFER_SIZE - ctx->buffer_len,
									 format, content_length, response->status, status_text, response->status,
									 status_text, description, host, port);
