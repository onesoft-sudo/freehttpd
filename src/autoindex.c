...
static int fhttpd_autoindex_sort (const struct dirent **e1, const struct dirent **e2)
{
                  ...
                  size_t filepath_len __attribute_maybe_unused__)
{
    ...
    int bytes_written = snprintf (html, html_len - 1, format, request->path, request->path, buf, request->conn->host,
                                  request->conn->port);
